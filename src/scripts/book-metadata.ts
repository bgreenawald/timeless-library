...
function initialize() {
  ...
  window.copyPromptToClipboard = function () {
    ...
    navigator.clipboard
      .writeText(window.currentRawPrompt)
      .then(() => {
  ...
  window.copyMetadataToClipboard = function () {
    ...
    navigator.clipboard
      .writeText(window.currentMetadataText)
      .then(() => {
  ...
  if (metadataButton) {
    metadataButton.addEventListener('click', () =>
      window.toggleMetadata(metadataButton as HTMLElement)
    );
  }
  ...
  if (promptModal) {
    ...
    promptModal
      .querySelector('button.text-gray-400')
      ?.addEventListener('click', window.closePromptModal);
  ...
  if (rawMetadataModal) {
    document
      .getElementById('copy-metadata-btn')
      ?.addEventListener('click', window.copyMetadataToClipboard);
    rawMetadataModal
      .querySelector('button.text-gray-400')
      ?.addEventListener('click', window.closeRawMetadataModal);
