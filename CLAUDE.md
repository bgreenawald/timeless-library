...
- **Run Jest tests**: `npm run test:jest` (for Node.js utilities and backend
  logic)
...
An Astro-based static site for modernized classic literature with GitHub
integration for versioned book content.

### Testing Strategy

- **Vitest**: Tests Astro components and frontend functionality
  (`src/components/__tests__/`)
- **Jest**: Tests Node.js utilities and backend logic (`src/lib/__tests__/`,
  `src/scripts/__tests__/`)
- **Happy DOM**: Test environment for component testing

The repo keeps both runners: Vitest is set up for Astro’s component pipeline and
happy-dom; Jest covers `src/lib` and scripts with the existing Node-focused
setup and `ts-jest`. Consolidating on Vitest alone is possible but would be a
larger migration (port mocks and config). Until then, run both suites before
release as documented in `AGENTS.md`.
...
- **`github.ts`**: GitHub API integration for fetching releases and book
  versions
- **`paths.ts`**: Dynamic path generation for Astro static site generation
- **`logger.ts`**: Winston-based logging (see `docs/LOGGING.md` for
  configuration)
...
- **Dynamic routing**: `[book].astro` and `[book]/[version].astro` for book and
  version pages
- **Book listing**: `books/index.astro` shows all books
- **Version comparison**: `[book]/[version]/diff.astro` for comparing book
  versions
